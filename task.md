...
> 🏁 **2026-07-05 建设期里程碑**：核心功能全上线、每日自愈稳定。「每日更新 + 排查日志管理」转入**用户另起的独立任务**。
> 注：归档当日又做了下方"信源统一重构"，项目持续演进中，非硬冻结。

## 当前：信源统一重构（owner: Claude Code，2026-07-05 晚，已上线）
1. ✅ 统一公司总表 `lib/companies.py`：投资标的=财报=企业新闻同一套基准
2. ✅ 三脚本从总表派生：股价19 / SEC财报10(补ARM/Oracle) / A股港股财报8(雅虎财务页) / 企业监控26 / AI媒体2分离
3. ✅ DeepSeek 入监控；新智元/机器之心 移入 AI 媒体；月之暗面等 6 家=关注·未持仓
4. ✅ 付费墙 `lib/paywall.py`：只砍硬墙外链、计费墙站保留；全链路清理含历史
5. ✅ NaN 崩溃根治 + 股票单只重试；dev 加完整信源总览
6. 线上验证通过并已 push 部署成功

## 未决 / 待办（2026-07-05）
- **ARM 财报**：SEC 拉取偶尔缺一份 form（10-K/10-Q），观察；SpaceX(SPCX) 无财报（非真上市）
- 付费墙硬墙名单可继续增删（用户可随时点名加/减）；A股/港股财报目前用雅虎财务页，若要官方公告（港交所/巨潮）需另接
- 关注·未持仓公司若转为持仓，补 ticker/cik 即自动带齐三件套（操作台已支持）
- 承前：api_server launchd 常驻 / 移动端实机 / 部署 flaky 靠重跑
